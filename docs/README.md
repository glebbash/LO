...
> For up-to-date examples see [examples](../examples) directory.
...
Top level expressions are expressions only allowed at the top level of the file (not inside functions).
...
Code expressions are expressions that are allowed inside function bodies.
...
Comments are allowed around between any tokens, but they might get moved during formatting into proper placing. See [formatter](#53-formatter) for more info.
...
> Compiles input file into a wasm module
>
...
> Prints inspection info in JSON format. Useful for IDE integrations.
>
...
> NOTE: this feature is WIP and does not support the full syntax yet
...
> NOTE: comments that are placed in "weird" places will be moved into closest "proper" places.
>
...
> NOTE: This feature will probably not get tested much as the main language is WASM.
